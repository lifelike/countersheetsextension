...
# Copyright 2008-2025 Pelle Nilsson and contributors
...
class CountersheetEffect(inkex.Effect, SvgOutputMixin):
    ...
    def generatecounter(self, c, rects, layer, colx, rowy, rotate):
        ...
        for p in c.parts:
            ...
            if group is None:
                ...
                sys.exit(
                    "Rectangle '%s' not in a group. Can not be template."
                    % rectname
                )
            ...
            textishnodes.extend(clone.xpath("//svg:flowSpan", namespaces=NSS))
            textishnodes.extend(clone.xpath("//svg:flowRoot", namespaces=NSS))
            ...
            for i in clone.xpath("//svg:image", namespaces=NSS):
                ...
                for glob, image in c.subst.items():
                    if fnmatch.fnmatchcase(imageid, glob):
                        ...
                        absref = i.get(
                            inkex.addNS("absref", "sodipodi"), image
                        )
                        href = self.make_image_href(
                            i.get(inkex.addNS("href", "xlink"), image)
                        )
                        i.set(
                            inkex.addNS("absref", "sodipodi"),
            ...
            ) = converter.SVG_to_dtl((-x, -y))
            self.translate_element(
                clone, source_layer_adjusted_x, source_layer_adjusted_y
            )
            ...
            if fnmatch.fnmatchcase(textid, glob):
                if subst.find("\\n") >= 0:
                    if t.tag == inkex.addNS("flowRoot", "svg"):
                        self.setMultilineFlowRoot(
                            t, textid, subst.split("\\n")
                        )
    ...
    def addbacks(self, layer, bstack, backxs, backys, docwidth, rects):
        ...
        for c, x, y in zip(bstack, backxs, backys):
            ...
            self.generatecounter(
                c.back, rects, layer, docwidth - x, y, self.options.rotatebacks
            )
    ...
    def export_using_inkscape(
        ...
        for id in ids:
            if (
                len(self.document.xpath("//*[@id='%s']" % id, namespaces=NSS))
                == 0
            ):
                continue
    ...
    def getbitmapfilename(self, id, directory, extension):
        return (
            os.path.join(os.path.abspath(directory), self.bitmapname + id)
            + "."
            + extension
        )
    ...
    def set_style_on_elements(self, element_ids, part, value):
        self.logwrite(
            "set_style_on_elements %r %s=%s\n" % (element_ids, part, value)
        )
        for element_id in element_ids:
            matching_elements = self.document.xpath(
                "//*[@id='%s']" % element_id, namespaces=NSS
            )
            if not matching_elements:
                return
            self.set_style(matching_elements[0], part, value)
    ...
    def exportSheetPDFs(self):
        self.logwrite(
            "exportSheetPDFs %s %d\n"
            % (self.options.pdfdir, len(self.cslayers))
        )
        if self.options.pdfdir and len(self.cslayers) > 0:
    ...
    def exportSheetBitmaps(self):
        if (
            self.options.bitmapsheetsdpi > 0
    ...
    def create_registrationline(self, x1, y1, x2, y2):
        self.logwrite(
            "create_registrationline %f,%f %f,%f\n" % (x1, y1, x2, y2)
        )
        return self.create_line(
            x1, y1, x2, y2, self.find_registration_line_style()
        )

    def find_style(self, styleid, failback):
        style_elements = self.document.xpath(
            "//*[@id='%s']" % styleid, namespaces=NSS
        )
    ...
    def add_registration_line_both_sides(
        self, x1, y1, x2, y2, layer, backlayer, docwidth
    ):
        self.add_registration_line(x1, y1, x2, y2, layer)
        if backlayer is not None and self.registrationmarksbothsides:
            self.add_registration_line(
                docwidth - x1, y1, docwidth - x2, y2, backlayer
            )

    def addregistrationmarks(
        self,
        xregistrationmarks,
        yregistrationmarks,
        position,
        ...
        if self.outlinemarks:
            ...
            if backlayer is not None:
                self.add_outlinemarks(
                    backlayer, docwidth - x1, y1, docwidth - x2, y2
                )
    ...
    def create_foldingline(self, docwidth, docheight):
        margin = max(
            self.registrationmarkslen + self.registrationmarksdist,
            self.outlinedist,
        )
        x1 = docwidth / 2
        y1 = margin
    ...
    def add_outlinemarks(self, layer, x1, y1, x2, y2):
        self.logwrite(
            "Outline rectangle around %f,%f %f,%f\n" % (x1, y1, x2, y2)
        )
        layer.append(self.create_registrationline(x1, y1, x2, y1))
        layer.append(self.create_registrationline(x1, y1, x1, y2))
        layer.append(self.create_registrationline(x1, y2, x2, y2))
        layer.append(self.create_registrationline(x2, y1, x2, y2))
    ...
    def calculateScale(self, svg):
        ...
        try:
            ...
            if viewbox:
                ...
                self.logwrite(
                    "SVG widthxheight: %sx%s\n" % (svgwidth, svgheight)
                )
                self.logwrite(
                    "SVG size in user-units: %fx%f\n"
                    % (svguuwidth, svguuheight)
                )
                xscale = (
                    self.svg.unittouu(svg.get("width"))
                    / vieww
                    / self.svg.unittouu("1px")
                )
                yscale = (
                    self.svg.unittouu(svg.get("height"))
                    / viewh
                    / self.svg.unittouu("1px")
                )
                self.xscale = xscale
            ...
            self.logwrite(
                "Failed to calculate document scale:\n%s\n" % repr(e)
            )
    ...
    def effect(self):
        ...
        self.fullregistrationmarks = (
            self.options.fullregistrationmarks == "true"
        )
        ...
        self.registrationmarkslen = self.from_len_arg(
            self.options.registrationmarkslen, "registration marks length"
        )
        ...
        self.outlinedist = self.from_len_arg(
            self.options.outlinedist, "outline distance"
        )
        self.spacing = self.from_len_arg(self.options.spacing, "spacing")
        self.backoffsetx = self.from_len_arg(
            self.options.backoffsetx, "back horizontal offset", True
        )
        self.backoffsety = self.from_len_arg(
            self.options.backoffsety, "back vertical offset", True
        )
        ...
        if not positions or len(positions) < 1:
            ...
            if self.oneside:
                layoutwidth /= 2.0
            positions = [Rectangle(0.0, 0.0, layoutwidth, docheight)]
            margin = max(
                self.registrationmarkslen + self.registrationmarksdist,
                self.outlinedist,
            )
        ...
        for i, c in enumerate(counters):
            ...
            while c.can_add_another():
                ...
                nr = nr + 1
                self.logwrite(
                    "laying out counter %d (nr %d/%r, c.nr %d)"
                    " (hasback: %s)\n"
                    % (i, nr, c.repeat.nr, c.repeat.keep_going, c.hasback)
                )
                ...
                width, height = self.generatecounter(
                    c,
                    rects,
                    layer,
                ...
                if (
                    ...
                    is_first_row = False
                    if (
                        nextrowy + height > positions[box].h + BOX_MARGIN
                        or c.endbox
                    ):
                        last_in_box = True
                        ...
                        if box == len(positions) and i < len(counters):
                            ...
                            if hasback:
                                ...
                                if not self.oneside:
                                    ...
                                    backlayer = self.create_backlayer(
                                        svg, suffix, csn
                                    )
                                if self.foldingline:
                                    self.add_foldingline(
                                        layer, docwidth, docheight
                                    )
                            svg.append(layer)
        ...
        if (
            len(xregistrationmarks) > 1 or len(yregistrationmarks) > 1
        ...
        if hasback:
            self.addbacks(backlayer, bstack, backxs, backys, docwidth, rects)
    ...
    def add_layer_backgrounds(self, layers, sheet_template, nrsheets):
        ...
        for target, nr in layers:
            ...
            string_replace_xml_text(background, "%SHEET%", str(nr))
            string_replace_xml_text(background, "%SHEETS%", str(nrsheets))
    ...
    def create_backlayer(self, svg, suffix, csn):
        backlayer = self.addLayer(svg, suffix, csn, "back")
        if self.backoffsetx != 0 or self.backoffsety != 0:
            self.translate_element(
                backlayer, self.backoffsetx, self.backoffsety
            )
        return backlayer
...
class CSVCounterDefinitionParser:
    ...
    def parse_row(self, row, factory):
        if self.is_counterrow(factory, row):
            ...
            return CSVCounterFactory(self.rects, self.defs, row, self.datadir)
        else:
    ...
    def must_parse_int(self, nrstr, endindex):
        try:
            ...
            sys.exit("Failed to parse repeat cell '%s'" % nrstr)
...
class CSVCounterFactory(CounterFactory):
    ...
    def parse_header(self, h):
        if self.iscopytoback(h):
            ...
            return AttributeLayout(h, self.rects, self.defs)
    ...
    def create_counter(self, repeat, row):
        ...
        for i, ho in enumerate(self.headers):
            ...
            if setting.back:
                if i >= len(row) or (
                    row[i] != "BACK" and not is_yes_value(row[i])
                ):
                    break
                c.hasback = True
                c = c.doublesided()
    ...
    def parse_defaultvalueheader(self, h):
        i = h.find("=")
        return DefaultValueLayoutDecorator(
            h[i + 1 :], self.parse_header(h[:i])
        )
...
class AttributeLayout:
    ...
    def color_lookup(self, color):
        found_id = self.defs.xpath("*[@id='%s']" % color, namespaces=NSS)
        found_href = self.defs.xpath(
            "*[@xlink:href='#%s']" % color, namespaces=NSS
        )
        if len(found_href) > 0:
    ...
    def getrefstyle(self, oldv, pname, value):
        [pstart, pend] = find_stylepart(oldv, pname)
...
class DocumentTopLeftCoordinateConverter:
    ...
    def dtl_to_SVG(self, dtl_point):
        ...
        """
        return (
            dtl_point[0] - self._transformX,
            dtl_point[1] - self._transformY,
        )

    def SVG_to_dtl(self, svg_point):
        return (
            svg_point[0] + self._transformX,
            svg_point[1] + self._transformY,
        )
...
def find_file(filename, extra_paths=None):
    ...
    for path in search_paths:
        ...
        sys.exit(
            "Unable to find file. Looked for:\n"
            "%s\n"
            ...
            % ("\n".join(search_paths), os.path.basename(filename))
        )
...
IGNORE_PATTERNS = [
    "did not resolve to a valid image file",
    ": dbind-WARNING **",
    ": WARNING **",
...
def get_layer(element, sourceElementId=None):
    ...
    if top_level_group is None:
        raise ValueError(
            "Unable to find layer for element [" + sourceElementId + "]"
        )
    else:
